...
const CONTACTFORM: FC = () => {
    ...
    return (
        <form action="https://formsubmit.co/muhammadxojadispatch@gmail.com" method="POST" className={styles.form}>
            <h2 className={styles.form_title}>Получить оптовый прайс</h2>

            <input className={styles.form_input} value={name} onChange={(e) => setName(e.target.value)} name="name" type="text" placeholder='Ф.И.О.' />
            <input className={styles.form_input} value={email} onChange={(e) => setEmail(e.target.value)} name="email" type="text" placeholder='@-mail' />
            <input className={styles.form_input} value={phone} onChange={(e) => setPhone(e.target.value)} name="phone" type="text" placeholder='Телефон (обязательно)' />
